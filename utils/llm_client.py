    ...
    async def chat(
        ...
        *temperature* overrides the value from settings when provided.
        ...
        for attempt in range(1, _MAX_RETRIES + 1):
            ...
            except _RETRYABLE + (Exception,) as exc:
                # litellm wraps provider errors — retry on transient failures,
                # but also catch generic Exception for litellm-specific errors
                # that don't map to openai exception types.
                is_retryable = (
